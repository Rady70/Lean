...
analytics) is implemented on branch `marketlab/single-anchor-research-account`
and proposed as a pull request against `master`, following this plan's section
...
authority. PR 2 changes no strategy formula and no PR 1 or PR 7 semantics. It
is proposed as a GitHub pull request against `master` and is not merged by this
work; PR 3, parameter research, the composed full-history data folder and the
frozen full-history baseline remain unimplemented.
...
PR 1 is implemented and merged (see the implementation note at the top); PR 2 is implemented (see the PR 2 implementation note at the top); PR 3 is the next implementation phase.
